Shader "Custom/FiveOctaveTwinklingGalaxy_MultiColor"
{
    Properties
    {
        ...
        _StarColor1 ("Star Color 1 (Blue/White)", Color) = (0.6, 0.8, 1.0, 1) 
        _StarColor2 ("Star Color 2 (Bright White)", Color) = (0.9, 0.9, 1.0, 1)
        _StarColor3 ("Star Color 3 (Warm Yellow)", Color) = (1.0, 0.8, 0.5, 1)
        _StarColor4 ("Star Color 4 (Red Dwarf)", Color) = (1.0, 0.4, 0.4, 1)
        ...
        _NebulaColor4 ("Nebula Highlight 3 (Magenta)", Color) = (0.9, 0.1, 0.5, 1)
        _NebulaColor5 ("Nebula Highlight 4 (Gold)", Color) = (0.8, 0.5, 0.1, 1)
        ...
        _BandingSmoothness ("Nebula Banding Smoothness", Range(0.001, 0.25)) = 0.02 // NEW SLIDER
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 _NebulaColor5;
            float _NebulaScale;
            float _NebulaIntensity;
            float _NebulaSpeed;
            float _BandingSmoothness; 
            ...
            // Upgraded FBM with 5 octaves and Domain Warping for wispy details
            float fbm(float3 x)
            {
                float v = 0.0;
                float a = 0.5;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                // ADJUSTABLE BANDING: Dynamic smoothstep margins based on inspector slider
                float t1 = 0.25;
                float t2 = 0.50;
                float t3 = 0.75;
                
                fixed4 multiColorHighlight = lerp(_NebulaColor2, _NebulaColor3, smoothstep(t1 - _BandingSmoothness, t1 + _BandingSmoothness, colorMap));
                multiColorHighlight = lerp(multiColorHighlight, _NebulaColor4, smoothstep(t2 - _BandingSmoothness, t2 + _BandingSmoothness, colorMap));
                multiColorHighlight = lerp(multiColorHighlight, _NebulaColor5, smoothstep(t3 - _BandingSmoothness, t3 + _BandingSmoothness, colorMap));
                ...
                // Stars retain smooth transitions so they don't look artificial
                float starRand = starMod1.x;
                fixed4 currentStarColor = lerp(_StarColor1, _StarColor2, smoothstep(0.0, 0.33, starRand));
                currentStarColor = lerp(currentStarColor, _StarColor3, smoothstep(0.33, 0.66, starRand));
                currentStarColor = lerp(currentStarColor, _StarColor4, smoothstep(0.66, 1.0, starRand));
